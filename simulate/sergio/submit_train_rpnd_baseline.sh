#!/usr/bin/env bash
#SBATCH --job-name=state_train_rpnd_baseline
#SBATCH --time=12:00:00
#SBATCH --nodes=1
#SBATCH --gpus=8
#SBATCH --ntasks-per-node=1
#SBATCH --output=slurm_logs/state_train_rpnd_baseline_%j.out
#SBATCH --error=slurm_logs/state_train_rpnd_baseline_%j.err

# rpnd_baseline: scratch baseline on Replogle-Nadig dataset, 50k steps, no pre-pre-training
# Reproducing ARC Institute ST-HVG-Replogle (arcinstitute/ST-HVG-Replogle) independently.
...
# Spec: plan3.md Section 3.2 (rpnd_baseline)
...
export WANDB_RUN_GROUP=rpnd_baseline
...
uv run state tx train \
  data.kwargs.toml_config_path="${SERGIO_DIR}/configs/rpnd_train.toml" \
