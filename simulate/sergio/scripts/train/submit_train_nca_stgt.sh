#!/usr/bin/env bash
#SBATCH --job-name=state_train_nca_stgt
#SBATCH --time=08:00:00
#SBATCH --nodes=1
#SBATCH --gpus=8
#SBATCH --ntasks-per-node=1
#SBATCH --output=slurm_logs/state_train_nca_stgt_%j.out
#SBATCH --error=slurm_logs/state_train_nca_stgt_%j.err

# nca_stgt: fine-tune from nca_ppt backbone on SERGIO_TGT_train_merged, 25k steps
# Spec: plan3.md Section 3.2 (nca_stgt)
#
# Step 1: remap NCA checkpoint keys to STATE's transformer_backbone namespace
...
export WANDB_RUN_GROUP=nca_stgt
...
# Step 1: remap NCA checkpoint → STATE backbone namespace
