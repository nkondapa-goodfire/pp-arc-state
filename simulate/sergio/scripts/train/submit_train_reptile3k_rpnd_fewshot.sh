#!/usr/bin/env bash
#SBATCH --job-name=state_train_reptile3k_rpnd_fewshot
#SBATCH --time=4:45:00
#SBATCH --nodes=1
#SBATCH --gpus=8
#SBATCH --ntasks-per-node=1
#SBATCH --output=slurm_logs/state_train_reptile3k_rpnd_fewshot_%j.out
#SBATCH --error=slurm_logs/state_train_reptile3k_rpnd_fewshot_%j.err

# reptile3k_rpnd_fewshot: fine-tune from sergio_ppt_reptile step=3000 checkpoint on replogle-nadig fewshot split, 50k steps
...
PPT_CKPT="/mnt/polished-lake/home/nkondapaneni/state_runs/sergio_ppt_reptile/checkpoints/step=step=3000.ckpt"

mkdir -p "${SERGIO_DIR}/slurm_logs"

export WANDB_RUN_GROUP=reptile3k_rpnd_fewshot
