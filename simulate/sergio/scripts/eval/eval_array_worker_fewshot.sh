#!/usr/bin/env bash
#SBATCH --job-name=eval_worker_fewshot
#SBATCH --time=0:30:00
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --ntasks-per-node=1
#SBATCH --output=slurm_logs/eval_worker_fewshot_%A_%a.out
#SBATCH --error=slurm_logs/eval_worker_fewshot_%A_%a.err

# Array worker for fewshot models — evaluates one checkpoint per task.
# Checkpoint list matches ckpt_steps used in rpnd_fewshot / spptv1_rpnd_fewshot / nca_rpnd_fewshot training.
...
CHECKPOINTS=(
    ...
    step=step=35000.ckpt
    step=step=40000.ckpt
    step=step=45000.ckpt
