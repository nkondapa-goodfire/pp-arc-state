#!/usr/bin/env bash
#SBATCH --job-name=eval_worker_stgt_v2
#SBATCH --time=0:30:00
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --ntasks-per-node=1
#SBATCH --output=slurm_logs/eval_worker_stgt_v2_%A_%a.out
#SBATCH --error=slurm_logs/eval_worker_stgt_v2_%A_%a.err

# Array worker for v2 SERGIO_TGT models (sergio_tgtv2, nca_stgt_v2).
# ckpt_steps: [200,400,600,800,1000,1200,1400,2000,2200,2400,2600,2800,3000,4000,8000,14000,20000,25000]
# ARRAY=0-17
#
...
#   COMPLETION_MARKER  filename that indicates eval is done (e.g. ER_size100_seed0103_agg_results.csv)
...
CHECKPOINTS=(
    ...
    step=step=4000.ckpt
    step=step=8000.ckpt
    step=step=14000.ckpt
    step=step=20000.ckpt
    step=step=25000.ckpt
)
