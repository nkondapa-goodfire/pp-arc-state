#!/usr/bin/env bash
#SBATCH --job-name=sample_eff_reptile1k
#SBATCH --array=0-4
#SBATCH --time=01:00:00
#SBATCH --nodes=1
#SBATCH --gpus=8
#SBATCH --ntasks-per-node=1
#SBATCH --output=slurm_logs/sample_eff_reptile1k_%A_%a.out
#SBATCH --error=slurm_logs/sample_eff_reptile1k_%A_%a.err

# Sample efficiency sweep initialised from sergio_ppt_reptile step=1000 checkpoint.
#
# Task index layout:
...
#
# Usage: sbatch sample_efficiency_reptile1k_worker.sh
...
INIT_CKPT="${STATE_RUNS}/sergio_ppt_reptile/checkpoints/step=step=1000.ckpt"
...
if [ "${SLURM_ARRAY_TASK_ID}" -eq 4 ]; then
    ...
    RUN_NAME="sample_eff_reptile1k_full"
    ...
    RUN_NAME="sample_eff_reptile1k_cpp${CELLS_PER_PERT}"
