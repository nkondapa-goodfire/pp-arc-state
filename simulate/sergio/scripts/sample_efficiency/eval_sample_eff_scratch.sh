#!/usr/bin/env bash
#SBATCH --job-name=eval_sample_eff_scratch
#SBATCH --array=0-4
#SBATCH --time=0:30:00
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --ntasks-per-node=1
#SBATCH --output=slurm_logs/eval_sample_eff_scratch_%A_%a.out
#SBATCH --error=slurm_logs/eval_sample_eff_scratch_%A_%a.err

# Evaluate sample efficiency runs at step=8000 for the scratch condition.
# Task layout:
#   0: sample_eff_scratch_cpp10
#   1: sample_eff_scratch_cpp25
#   2: sample_eff_scratch_cpp50
#   3: sample_eff_scratch_cpp100
#   4: sergio_tgtv2  (full data)
...
RUN_DIRS=(
    "${STATE_RUNS}/sample_eff_scratch_cpp10"
    "${STATE_RUNS}/sample_eff_scratch_cpp25"
    "${STATE_RUNS}/sample_eff_scratch_cpp50"
    "${STATE_RUNS}/sample_eff_scratch_cpp100"
    "${STATE_RUNS}/sergio_tgtv2"
)
