...
WORKER="${SERGIO_DIR}/eval_array_worker_fewshot.sh"
...
JOB2=$(sbatch --array="${ARRAY}" \
    --dependency="afterany:378530" \
    --export="RUN_DIR=${STATE_RUNS}/spptv1_rpnd_fewshot,TEST_TOML=${TEST_TOML},COMPLETION_MARKER=${COMPLETION_MARKER}" \
    --job-name="eval_spptv1_rpnd_fewshot" \
    "${WORKER}" | awk '{print $4}')
echo "spptv1_rpnd_fewshot eval:  ${JOB2} (after train 378530)"
